...
inline size_t ByteStream::_available_output(const size_t len) const {
    return len > this->buffer_size() ? this->buffer_size() : len;
}

inline void ByteStream::_pop(const size_t size) {
    this->_buffer = this->_buffer.substr(size, this->buffer_size());
    this->_bytes_read += size;
}

inline string ByteStream::_peek(const size_t size) const { return this->_buffer.substr(0, size); }

ByteStream::ByteStream(const size_t capacity)
    : _capacity(capacity), _bytes_written(0), _bytes_read(0), _input_ended(false), _buffer(""), _error(false) {}

size_t ByteStream::write(const string &data) {
    const size_t size = data.size() > this->remaining_capacity() ? this->remaining_capacity() : data.size();
...
string ByteStream::peek_output(const size_t len) const { return this->_peek(this->_available_output(len)); }

//! \param[in] len bytes will be removed from the output side of the buffer
void ByteStream::pop_output(const size_t len) { this->_pop(this->_available_output(len)); }

//! Read (i.e., copy and then pop) the next "len" bytes of the stream
//! \param[in] len bytes will be popped and returned
//! \returns a string
std::string ByteStream::read(const size_t len) {
    const size_t size = this->_available_output(len);
