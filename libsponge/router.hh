...
class Router {
    ...
    class ForwardingTableEntry {
      ...
      public:
        ForwardingTableEntry(uint32_t prefix, uint8_t length, std::optional<Address> next, size_t interface)
            : route_prefix(prefix), prefix_length(length), next_hop(next), interface_num(interface){};

        uint32_t get_route_prefix() { return route_prefix; }
        uint8_t get_prefix_length() { return prefix_length; }
        std::optional<Address> get_next_hop() { return next_hop; }
        size_t get_interface_num() { return interface_num; }
        // creates proper mask for prefix comparison
        uint32_t get_mask() { return (prefix_length) ? ~((1 << (32 - prefix_length)) - 1) : 0; }
    };

    // mapping between outgoing interface and network number
    std::vector<ForwardingTableEntry> forwarding_table{};
